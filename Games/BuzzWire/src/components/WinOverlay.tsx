...
import { playWinChime } from '../lib/soundEffects'
...
export function WinOverlay({ onPlayAgain, onChangeDifficulty }: WinOverlayProps) {
  // Confetti in the theme's own colours, so winning looks like the world the player chose.
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-6 overflow-hidden bg-pz-overlay p-6">
      {confetti.map((piece) => (
        <span
          ...
          style={{
            ...
            animation: `buzzwire-confetti-fall ${piece.duration}s linear ${piece.delay}s 1`,
          }}
        />
      ))}
      ...
      <div className="relative flex flex-col items-center gap-4 rounded-2xl bg-pz-surface p-8 text-center shadow-2xl ring-1 ring-pz-ring">
        <h2 className="text-3xl font-bold text-pz-ink">You made it! 🎉</h2>
      ...
      <style>{`
        @keyframes buzzwire-confetti-fall {
